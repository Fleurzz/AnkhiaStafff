...
public class Titles {
    ...
    private static Class<?> getNMSClass(String name) throws ClassNotFoundException {
        Class<?> cachedClass = (Class)NMS_CLASS_CACHE.get(name);
        if (cachedClass != null) {
            return cachedClass;
        } else {
    ...
    public static void sendTitle(Player player, String title, String subtitle, int fadeIn, int stay, int fadeOut) {
        if (player != null && title != null) {
            ...
            try {
                ...
                Object chatSubtitle = subtitle != null ? getNMSClass("IChatBaseComponent").getDeclaredClasses()[0].getMethod("a", String.class).invoke((Object)null, "{\"text\":\"" + subtitle + "\"}") : null;
                ...
                Object titlePacket = titleConstructor.newInstance(getNMSClass("PacketPlayOutTitle").getDeclaredClasses()[0].getField("TITLE").get((Object)null), chatTitle, fadeIn, stay, fadeOut);
                Object subtitlePacket = subtitle != null ? titleConstructor.newInstance(getNMSClass("PacketPlayOutTitle").getDeclaredClasses()[0].getField("SUBTITLE").get((Object)null), chatSubtitle, fadeIn, stay, fadeOut) : null;
    ...
    private static Method getChatComponentMethod(Class<?> clazz) throws NoSuchMethodException {
        Method method = (Method)CHAT_COMPONENT_METHOD_CACHE.get(clazz);
        if (method == null) {
    ...
    private static Constructor<?> getChatPacketConstructor(Class<?> clazz) throws NoSuchMethodException, SecurityException, ClassNotFoundException {
        Constructor<?> constructor = (Constructor)CHAT_PACKET_CONSTRUCTOR_CACHE.get(clazz);
        if (constructor == null) {
            constructor = clazz.getConstructor(getNMSClass("IChatBaseComponent"), Byte.TYPE);
    ...
    public static void sendActionBar(Player player, String message) {
        if (player != null && message != null) {
            ...
            try {
                ...
                Object chatComponent = chatComponentMethod.invoke((Object)null, "{\"text\":\"" + message + "\"}");
                Class<?> packetClass = getNMSClass("PacketPlayOutChat");
                Constructor<?> actionBarConstructor = getChatPacketConstructor(packetClass);
                Object actionBarPacket = actionBarConstructor.newInstance(chatComponent, 2);
